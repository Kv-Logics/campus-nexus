...
2. **Zero-VPN Campus Intranet Mesh Gateway (Reverse WebSocket Tunnel)**:  
   University portals, internal datasets, files, and lab resources are frequently locked behind campus network firewalls (`10.x.x.x` or `.internal` subnets), completely inaccessible to students or faculty working remotely without complex VPN clients. Campus Nexus provides a browser-based reverse tunnel: a trusted peer connected to campus Wi-Fi (e.g., a student's mobile phone) opens a lightweight, isolated relay page (`relay.html`). The peer establishes a persistent, bi-directional WebSocket reverse tunnel back to the gateway. External users can then remotely access, fetch, and browse intranet-only files, internal portals, and private network data on demand, routed seamlessly through the peer's browser.
